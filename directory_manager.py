...
class Thread (threading.Thread):
    def __init__(self, pool, method, param=()):
    ...
    def run(self):
        ...
        # After execution, remove it from thread pool
        self.thread_pool.remove(self)


class TalkToFTPThreadify(TalkToFTP):
    def __init__(self, ftp_website):
        TalkToFTP.__init__(self, ftp_website)

    def create_folder_threadify(self, param):
        folder = param[0]
        self.ftp.mkd(folder)
        Logger.log_info("Folder created : " + folder)

    def remove_folder_threadify(self, param):
        folder = param[0]
        self.ftp.rmd(folder)
        Logger.log_info("Folder removed : " + folder)

    def file_transfer_threadify(self, param):
        path, srv_path, file_name = param
    ...
    def remove_file_threadify(self, param):
        file = param[0]
...
class DirectoryManager:
    def __init__(self, ftp_website, directory, depth, excluded_extensions, max_thread_num):
        ...
        # FTP instance
        self.ftp = TalkToFTPThreadify(ftp_website)
        ...
        if not self.ftp.if_exist(self.ftp.directory, self.ftp.get_folder_content(directory_split)):
            ...
            self.ftp.connect()
            t = Thread(self.thread_pool, self.ftp.create_folder_threadify, (self.ftp.directory))
                ...
                if self.is_superior_max_depth(folder_path) is False:
                    ...
                    if folder_path not in self.synchronize_dict.keys():
                        ...
                        if not self.ftp.if_exist(srv_full_path, self.ftp.get_folder_content(directory_split)):
                            ...
                            self.ftp = TalkToFTPThreadify(self.ftp_website)
                            self.ftp.connect()
                            t = Thread(self.thread_pool, self.ftp.create_folder_threadify, (srv_full_path))
                            self.thread_pool.append(t)
                ...
                if self.is_superior_max_depth(file_path) is False and \
                    ...
                    if file_path in self.synchronize_dict.keys():
                        ...
                        if self.synchronize_dict[file_path].update_instance() == 1:
                            ...
                            self.ftp = TalkToFTPThreadify(self.ftp_website)
                            self.ftp.connect()
                            t = Thread(self.thread_pool, self.ftp.remove_file_threadify, (srv_full_path))
                            self.thread_pool.append(t)
                            #self.ftp.remove_file(srv_full_path)
                            t.start()
                            # update this file on the FTP server
                            t = Thread(self.thread_pool, self.ftp.file_transfer_threadify, (path_file, srv_full_path, file_name))
                            self.thread_pool.append(t)
    ...
    def remove_all_in_directory(self, removed_directory, srv_full_path, path_removed_list):
        ...
        for i in range(len(sorted_containers)-1, -1, -1):
            for to_delete in sorted_containers[i]:
                ...
                if isinstance(self.synchronize_dict[to_delete], File):
                    self.ftp = TalkToFTPThreadify(self.ftp_website)
                    self.ftp.connect()
                    t = Thread(self.thread_pool, self.ftp.remove_file_threadify, (to_delete_ftp))
                    self.thread_pool.append(t)
                    t.start()
        ...
        self.ftp = TalkToFTPThreadify(self.ftp_website)
        self.ftp.connect()
        t = Thread(self.thread_pool, self.ftp.remove_folder_threadify, (srv_full_path))
        self.thread_pool.append(t)
        t.start()
